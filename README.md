...
BioContext locates a small, 10x-shrunk **Reference Image** inside a much
larger **Search Image** of wafer-inspection-style imagery, and is
specifically designed to resist the false matches created by highly
repetitive DRAM / FinFET periodic structure.
...
- a **Reference Image** that is a **10x shrunk** crop of *some* location inside
  the Search Image,
...
1. **Repetitive structure.** DRAM bit-cell arrays and FinFET fin arrays are
   periodic by design — the same local motif appears at hundreds of
   locations, so naive template matching regularly locks onto the wrong,
...
1. **Retina-inspired center-surround processing.** Retinal ganglion cells
   ...
   exactly what's needed to tell apart cells that otherwise look identical.
2. **Ant-inspired visual navigation.** Desert ants don't recognize a
   location from one memorized ground-level snapshot; they compare the
   *panorama around them* to a remembered panorama. We use the same idea:
   instead of trusting a single best local match, we keep several plausible
   candidate locations and re-rank them by how well the **surrounding
   context** — not just the bare repeating motif — agrees with the
   reference's own surrounding context.
...
| `retina_only` | Center-surround (DoG) preprocessing + single best peak. Isolates the retina component. |
| `context_only` | Multi-candidate generation (raw pixels) + ant-inspired surrounding-context verification, **no** DoG. Isolates the ant component. |
...
candidate generation. It measurably hurt accuracy: the reference is
captured at 1/10th resolution, so a tiny template correlated against a
heavily high-pass-filtered representation is dominated by a handful of edge
pixels and is very sensitive to small rotation/scale error. The retina
component is far more useful as an **independent evidence channel** applied
*after* candidates are proposed — it tells you whether a candidate's
neighborhood shares the reference's non-periodic micro-structure, which raw
correlation alone can't exploit. This is documented in code and is exactly
why the ablations exist: to show what each biological component
contributes, and where it doesn't help.

### "Center-most match" tie-break

When multiple candidates score within a small tolerance of the best score
**and** are spatially close to it (same physical cluster, not a
coincidentally similar-scoring decoy elsewhere in the image), we return the
candidate closest to the centroid of that tied cluster rather than an
arbitrary one. See `biocontext/methods/common.py::pick_center_most`.
...
biocontext/
  ...
  inference.py              # clean CLI + Python inference interface
  demo.py                    # DEMO MODE: one-command, screen-recordable end-to-end run
...
results/
  ...
  figures/                 # generated comparison plots + demo visualizations
demo_output/                # generated fresh each run of `python -m biocontext.demo` (gitignored)
requirements.txt
RUN_DEMO.md                 # exact command sequence to record the technical demo video
```
...
### Demo Mode (for screen-recording the technical demo)

```bash
python -m biocontext.demo
```

Runs the whole story end-to-end — dataset generation, reference/search
images, baseline, BioContext, side-by-side comparison, and a live benchmark
sweep — saving every result as a labeled PNG under `demo_output/` (and
auto-opening each one so it's on screen as it's produced). See
**`RUN_DEMO.md`** for the exact from-clone command sequence to record.
...
repetitive semiconductor layouts rather than claiming physical accuracy:

- A large **Search Image** built from geometric primitives: a repeating
  DRAM/FinFET-style unit cell (identical motif at every grid position —
  the source of false matches), optional **via/contact dots** layered onto
  cells, optional **repeated metal lines** (bitline/wordline-style full-row
  strokes), plus a sparse set of larger, spatially-unique "context markers"
  (analogous to rare defects / alignment features) that make each cell
  neighborhood's *surrounding* context locally unique even though the
  *cell itself* repeats everywhere. `CaseConfig.structure_type` selects
  `"cells"`, `"lines"`, `"vias"`, or `"mixed"`.
- **SEM-like rendering effects** applied on top (`CaseConfig.sem_effects`,
  on by default): a linear illumination gradient, radial vignette falloff,
  ...
  reference. These don't claim to be physically accurate SEM captures;
  they exist to make the imagery look like an inspection frame rather than
  a flat vector diagram, and to add the low-level nuisance variation a
  real detector has to be robust to.
...
- Every generated case also records the coordinates of visually-identical
  **decoy cells** elsewhere in the image, so we know exactly how ambiguous
  each case is, and `save_case_artifacts()` writes the actual rendered
...
position** (center / edge / corner / random), and a block of **combined
hard cases** stacking several nuisances at once — 64 cases total at the
default `n_per_factor=10`.
...
- On **isolated, single-nuisance cases** (scale error alone, noise alone,
  ...
  near-perfect, and BioContext matches it almost exactly — it's designed to
  fall back to baseline-equivalent behavior when the top match is
  unambiguous, not to override a confident correct answer.
- **BioContext gives its clearest, most important win on the `combined`
  block** — multiple nuisances stacked at once (rotation + noise + blur +
  scale error + high repetition), which is the regime closest to a real
  noisy inspection scenario and the whole point of the challenge: mean
  error **110.9px vs baseline's 167.8px**, success rate **30% vs 20%**.
  This is exactly where surrounding-context verification is supposed to
  help, and it does.
- On **isolated rotation**, the classical baseline and ant-context-only
  ...
  signal when rotation is the *only* nuisance — the retina (DoG) channel
  is somewhat rotation-sensitive at small kernel sizes and can inject
  noise into an otherwise-easy decision. We report this plainly rather
  than hide it; it's a genuine, documented limitation (see §8).
- **`retina_only` is consistently the weakest ablation on its own** —
  confirming our design note in §2: center-surround filtering is valuable
  as an auxiliary evidence channel, not as the sole basis for candidate
  detection on a tiny, 10x-shrunk template.

### Qualitative demo case — the headline failure mode

A hand-picked hard case (seed 116): 92% repetition density, 3° rotation,
noise σ=3, mixed cell/via/line structure, 1127 visually-identical decoy
cells in the search image.
...
rescoring + context verification together) recovers the correct location.
Four-way side-by-side (baseline / retina-only / context-only / biocontext)
on the same case: `results/figures/demo_method_comparison.png`.
...
- The dataset is synthetic. It's built to stress-test the *specific*
  failure mode described in the challenge (periodic structure + shrunk
  reference), not to reproduce real SEM/optical-inspection pixel
...
- `pick_center_most`'s tie-break radius and score-tolerance are fixed
  heuristics (`rel_epsilon=0.03`, `radius = 1.5x` bbox size); they were
  tuned qualitatively against the benchmark, not learned.
...
(`CaseConfig.seed`); `build_benchmark_suite` assigns seeds deterministically.
Re-running `python -m biocontext.eval.benchmark` regenerates identical
`results/tables/benchmark_raw.csv` bit-for-bit on the same package
versions. All dependencies and exact versions used are pinned as lower
bounds in `requirements.txt`.

## 10. Team / hackathon context

Built for the **Applied Materials SEMICON India Hackathon 2026 —
Drift-Sense** track. See `LICENSE` (MIT).
